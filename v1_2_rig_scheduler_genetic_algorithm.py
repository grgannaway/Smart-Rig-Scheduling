...
----------------------
* **Pre-approved pads**: Pads with a non-null ``mandatory_date`` are treated as
  pre-approved. They skip land, permitting, pad construction, midstream, and
  overland milestones entirely — entering the simulator at WAITING_DRILL on
  their mandatory date. Only rig + frac crew resources and drill + frac capex
  are consumed.  The GA chromosome excludes pre-approved pads (they are fixed).
* **Year 1 capex override**: ``SimConfig.year_1_capex_limit_mm`` provides an
  explicit ceiling for year 0 (sim start -> 12/31) that replaces the CAGR formula.
...
class SimConfig:
    ...
    # ---- Year 1 capex override (v1.2) -------------------------------------
    # If set (not None), this value replaces the CAGR-formula limit for year 0
    # (simulation start date -> 12/31 of that year).  Useful when pre-approved
    # pads drive a different capital profile in year 1.
    year_1_capex_limit_mm: Optional[float] = None
    ...
    def capex_limit_for_year(self, year: int) -> float:
        """Annual capex limit applying CAGR.  v1.2: year 0 can be overridden."""
        if year == 0 and self.year_1_capex_limit_mm is not None:
            return self.year_1_capex_limit_mm
...
def load_pads_from_csv(filepath: str, sim_start_date: str = "2026-01-01") -> List[WellPad]:
    ...
    for idx, row in df.iterrows():
        ...
        if md_raw is not None and pd.notna(md_raw) and str(md_raw).strip() != "":
            try:
                # v1.2: allow negative values — pre-approved pads that started
                # drilling before the sim start date will have mandatory_start_day < 0.
                mandatory_day = int((pd.Timestamp(str(md_raw).strip()) - ref_date).days)
            except Exception:
                mandatory_day = None
        pads.append(WellPad(
...
class OrderedDrillingSimulator:
    def __init__(self, pads: List[WellPad], config: SimConfig,
        ...
        if pad_order is not None:
            ...
            # Pre-approved pads are NOT in pad_order; they sort to the end
            # of the sequence (their actual scheduling is driven by
            # mandatory_start_day in the daily loop, not sequence_number).
    ...
    def run(self) -> pd.DataFrame:
        for day in range(self.config.simulation_days):
            ...
            # v1.2: mandatory pads get rig priority over non-mandatory
            ...
            for pad in wd:
                ...
                if cost > 0 and not self._can_admit(day, pad.drill_days, cost)[0]:
                    continue
                # v1.2: don't start a non-mandatory pad if its drill window
                # would block an upcoming pre-approved mandatory pad.
...
def _evaluate_ordering(
    ...
    for p in sim.pads:
        if p.is_mandatory and p.mandatory_start_day is not None:
            ...
            # — they can never "slip", so skip the violation check.
            if p.mandatory_start_day < 0:
                continue
            # v1.2: for pre-approved pads, the mandatory date is the drill
            # start day (not land start).
            if p.is_pre_approved:
            ...
            if actual_start > p.mandatory_start_day + max(1, grace):
                ...
                # Show actual rigs in use on the mandatory date for diagnostics
                mday = int(p.mandatory_start_day)
                if 0 <= mday < len(sim.daily_rig_use):
                    rigs_at_entry = sim.daily_rig_use[mday]
                else:
                    rigs_at_entry = "?"
                detail = (f"{p.name}: {ms_label}={actual_start if actual_start < 10**9 else 'NEVER'} "
                          f"vs mandatory_day={p.mandatory_start_day} "
                          f"(slip={slip if actual_start < 10**9 else 'INF'}d, "
                          f"status={status_str}, "
...
def main():
    ...
    config = SimConfig(
        ...
        year_1_capex_limit_mm=None,  # v1.2: set to override year 0 ceiling (e.g. 200.0)
